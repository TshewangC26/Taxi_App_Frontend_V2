...
class _AdminRoutesScreenState extends State<AdminRoutesScreen>
  ...
  Future<void> _loadRoutes() async {
    ...
    try {
      final response = await _apiService.get('/admin/routes');
      setState(() {
  ...
  void _showRouteDialog({dynamic route}) {
    ...
    final pickupCtrl = TextEditingController(
        text: route?['pickup_location'] ?? '');
    final dropoffCtrl = TextEditingController(
        text: route?['dropoff_location'] ?? '');
    final price4 = TextEditingController(
        text: route?['price_4_seater']?.toString() ?? '');
    final price7 = TextEditingController(
        text: route?['price_7_seater']?.toString() ?? '');
    final price8 = TextEditingController(
        text: route?['price_8_seater']?.toString() ?? '');
                    ...
                    Expanded(
                      child: ElevatedButton(
                        onPressed: () async {
                          ...
                          if (pickupCtrl.text.trim() ==
                              dropoffCtrl.text.trim()) {
                            ScaffoldMessenger.of(context)
                                .showSnackBar(SnackBar(
                              content: const Text(
                          ...
                          try {
                            final data = {
                              ...
                              'dropoff_location':
                                  dropoffCtrl.text.trim(),
                              'price_4_seater':
                                  double.tryParse(price4.text) ??
                                      0,
                              'price_7_seater':
                                  double.tryParse(price7.text) ??
                                      0,
                              'price_8_seater':
                                  double.tryParse(price8.text) ??
                                      0,
                  ...
                  child: Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text('Tap + Add Route to create one',
                            style: TextStyle(
                                fontSize: 13,
            ...
            Row(
              ...
              children: [
                _priceChip(
                  ...
                  iconColor: const Color(0xFF1565C0),
                  seats: '4',
                  price: route['price_4_seater']?.toString() ?? '0',
                ),
                Container(
                ...
                _priceChip(
                  ...
                  iconColor: const Color(0xFF6A1B9A),
                  seats: '7',
                  price: route['price_7_seater']?.toString() ?? '0',
                ),
                Container(
                    width: 1, height: 36, color: Colors.grey.shade100),
                _priceChip(
                  ...
                  iconColor: const Color(0xFF00695C),
                  seats: '8',
                  price: route['price_8_seater']?.toString() ?? '0',
                ),
              ],
            ),
