...
class _DriverAvailableRidesScreenState
        ...
        for (final booking in dp.myActiveBookings) {
          ...
          if (passengerId != null &&
            ...
            final pLat = double.tryParse(
                booking['passenger_latitude']?.toString() ?? '');
            final pLng = double.tryParse(
                booking['passenger_longitude']?.toString() ?? '');
            if (pLat != null && pLng != null) {
              _updateMarkers(
                  staticPassengerLat: pLat, staticPassengerLng: pLng);
            }
            break;
          }
        }
    ...
    switch (index) {
      case 0:
        Navigator.pushAndRemoveUntil(
          ...
          PageRouteBuilder(
            pageBuilder: (_, a, __) =>
                FadeTransition(opacity: a, child: const DriverHomeScreen()),
        ...
        Navigator.pushReplacement(
          ...
          PageRouteBuilder(
            pageBuilder: (_, a, __) => FadeTransition(
                opacity: a, child: const DriverMyRidesScreen()),
            transitionDuration: const Duration(milliseconds: 300),
          ),
        );
        break;
      case 3:
        Navigator.pushReplacement(
          ...
          PageRouteBuilder(
            pageBuilder: (_, a, __) => FadeTransition(
                opacity: a, child: const DriverEarningsScreen()),
            transitionDuration: const Duration(milliseconds: 300),
          ),
        );
        break;
      case 4:
        Navigator.pushReplacement(
          ...
          PageRouteBuilder(
            pageBuilder: (_, a, __) => FadeTransition(
                opacity: a, child: const DriverProfileScreen()),
            transitionDuration: const Duration(milliseconds: 300),
          ),
        );
        break;
  ...
  void _goToMyRides() {
    Navigator.pushReplacement(
      ...
      PageRouteBuilder(
        pageBuilder: (_, a, __) =>
            FadeTransition(opacity: a, child: const DriverMyRidesScreen()),
        transitionDuration: const Duration(milliseconds: 300),
      ),
    );
  }
            ...
            children: [
              Stack(alignment: Alignment.center, children: [
                Container(
                    ...
                    height: 80,
                    decoration: BoxDecoration(
                        color: confirmColor.withOpacity(0.07),
                ...
                Container(
                  ...
                  height: 46,
                  decoration: BoxDecoration(
                      color: confirmColor, shape: BoxShape.circle),
    ...
    try {
      ...
      _driverLocationSubscription = Geolocator.getPositionStream(
        ...
        if (_passengerLat != null && _passengerLng != null) {
          _drawRoute(LatLng(pos.latitude, pos.longitude),
    ...
    try {
      ...
      if (polylinePoints.isNotEmpty) {
        _mapController?.animateCamera(CameraUpdate.newLatLngBounds(
            _getBounds(origin, destination), 80));
      }
    } catch (e) {
      print('Route error: $e');
    }
  ...
  void _updateMarkers(
      {double? staticPassengerLat, double? staticPassengerLng}) {
    if (!mounted) return;
  ...
  void _showPaymentDialog(
      BuildContext context, dynamic booking, DriverProvider dp) {
            ...
            children: [
              Container(
                ...
                child: Icon(Icons.payment_rounded,
                    color: Colors.yellow[800], size: 26),
              ),
              ...
              Text('How did the passenger pay?',
              ...
              Row(children: [
                Expanded(
                  child: OutlinedButton.icon(
                    onPressed: () {
                      Navigator.pop(ctx);
                      _showCashConfirmation(context, amount);
                    },
                ...
                Expanded(
                  child: ElevatedButton.icon(
                    onPressed: () {
                      Navigator.pop(ctx);
                      _showOnlinePaymentDetails(context, amount,
                          accountNumber, mobileNumber, qrCodeUrl);
                    },
  ...
  Widget build(BuildContext context) {
    ...
    final totalPending =
        dp.nowBookings.length + dp.scheduledBookings.length;
    final currentCount =
        dp.nowBookings.length + dp.myActiveBookings.length;
                      ...
                      if (dp.scheduledBookings.isNotEmpty) ...[
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                              color: Colors.yellow[800],
                              borderRadius: BorderRadius.circular(10)),
                          child: Text('${dp.scheduledBookings.length}',
                              style: const TextStyle(
                                  color: Colors.white, fontSize: 10)),
                        ),
                      ],
        ...
        children: [
          ...
          Container(
            ...
            child: Row(children: [
              ...
              const SizedBox(width: 14),
              Container(
                  width: 18, height: 3, color: Colors.yellow[800]),
              const SizedBox(width: 5),
              Text('Route',
                  style:
                      TextStyle(color: Colors.grey[600], fontSize: 12)),
  ...
  Widget _buildCurrentTab(BuildContext context, DriverProvider dp) {
    final hasAnything =
        dp.nowBookings.isNotEmpty || dp.myActiveBookings.isNotEmpty;

    if (!hasAnything) {
      return _buildEmptyState(
        icon: Icons.directions_car_rounded,
        message: 'No current rides',
        hint: 'Make sure you are online to see rides',
      );
    }
        ...
        child: Column(
          ...
          children: [
            if (dp.myActiveBookings.isNotEmpty) ...[
              _buildSectionHeader('My Active Ride',
                  const Color(0xFFE65100), dp.myActiveBookings.length),
              const SizedBox(height: 10),
              ...dp.myActiveBookings.map((b) =>
                  _buildBookingCard(context, b, dp, cardType: 'active')),
              const SizedBox(height: 20),
            ],
            if (dp.nowBookings.isNotEmpty) ...[
              _buildSectionHeader('Pending Bookings',
                  const Color(0xFF1565C0), dp.nowBookings.length),
              const SizedBox(height: 10),
              ...dp.nowBookings.map((b) =>
                  _buildBookingCard(context, b, dp, cardType: 'pending')),
            ],
          ],
        ),
      ),
  ...
  Widget _buildScheduledTab(BuildContext context, DriverProvider dp) {
    if (dp.scheduledBookings.isEmpty) {
      return _buildEmptyState(
        ...
        message: 'No scheduled rides',
        hint: 'Scheduled rides will appear here',
      );
    }
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) => _buildBookingCard(
            context, dp.scheduledBookings[index], dp,
            cardType: 'scheduled'),
      ),
    );
  }
  ...
  Widget _buildBookingCard(
    ...
    final passengerLat = double.tryParse(
        booking['passenger_latitude']?.toString() ?? '');
    final passengerLng = double.tryParse(
        booking['passenger_longitude']?.toString() ?? '');
    final passengerId = booking['passenger_id'];
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                Container(
                  ...
                  child: Text(
                    status.isEmpty ? 'Pending' : _capitalize(status),
                    style: TextStyle(
                        color: statusColor,
                        fontWeight: FontWeight.w700,
                        fontSize: 12),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          '${_formatScheduledDate(booking['scheduled_date'])}  •  '
            ...
            Row(children: [
              ...
              const SizedBox(width: 10),
              Expanded(
                  child: Text(booking['pickup_location'] ?? '',
                      style: const TextStyle(
                          fontWeight: FontWeight.w600,
                          fontSize: 13,
                          color: Colors.black87))),
            ...
            Row(children: [
              ...
              const SizedBox(width: 10),
              Expanded(
                  child: Text(booking['dropoff_location'] ?? '',
                      style: const TextStyle(
                          fontWeight: FontWeight.w600,
                          fontSize: 13,
                          color: Colors.black87))),
                  ...
                  child: OutlinedButton.icon(
                    onPressed: () async {
                      ...
                      if (confirmed == true && context.mounted) {
                        await dp.cancelBooking(booking['id']);
                      }
                    },
                  ...
                  child: ElevatedButton.icon(
                    onPressed: () async {
                      final confirmed = await _confirmDialog(context,
                          ...
                          message:
                              'Accept this booking? You will be navigated to My Rides.',
                          confirmText: 'Yes, Accept',
                          confirmColor: const Color(0xFF2E7D32),
                          iconData: Icons.check_circle_rounded);
                      if (confirmed == true && context.mounted) {
                        final success =
                            await dp.acceptBooking(booking['id']);
                        if (success && context.mounted) {
                          if (passengerLat != null &&
                              passengerLng != null) {
                            _updateMarkers(
                                staticPassengerLat: passengerLat,
                                staticPassengerLng: passengerLng);
                            if (_driverLat != null) {
                              _drawRoute(
                  ...
                  child: ElevatedButton.icon(
                    onPressed: () async {
                      final confirmed = await _confirmDialog(context,
                          ...
                          message:
                              'Your status stays Available until the ride starts.',
                          confirmText: 'Yes, Accept',
                          confirmColor: const Color(0xFF5E35B1),
                          iconData: Icons.calendar_today_rounded);
                      if (confirmed == true && context.mounted) {
                        final success = await dp
                            .acceptScheduledBooking(booking['id']);
                        if (success && context.mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                                content: const Text(
                                    'Scheduled ride accepted! Check My Rides.'),
                                backgroundColor: Colors.yellow[800]),
                          );
                          _goToMyRides();
                        }
                  ...
                  onPressed: () async {
                    ...
                    if (success && context.mounted) {
                      ...
                      if (context.mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                              content: const Text('Ride started!'),
                              backgroundColor: Colors.yellow[800]),
                        );
                      }
                    }
            ...
            // ACTIVE in_progress → Complete Ride
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  onPressed: () async {
                    ...
                    if (confirmed == true && context.mounted) {
                      final success =
                          await dp.completeRide(booking['id']);
                      if (success && context.mounted) {
                        _passengerLocationSubscription?.cancel();
                        setState(() {
                          _passengerLat = null;
                        ...
                        if (context.mounted) {
                          _showPaymentDialog(context, booking, dp);
                        }
                      }
                    }
                  },
